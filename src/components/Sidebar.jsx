...
export default function Sidebar() {
  ...
  return (
    ...
    >
      {/* BOTÃO FLUTUANTE IGUAL AO PRINT (Na borda da sidebar) */}
      <button
        onClick={() => setColapsado(!colapsado)}
        className="absolute -right-3.5 top-7 w-7 h-7 rounded-full bg-blue-600 hover:bg-blue-500 text-white flex items-center justify-center shadow-lg cursor-pointer z-50 border-2 border-slate-900 transition-transform active:scale-95"
      ...
      {/* WRAPPER INTERNO (Protege contra qualquer scroll horizontal) */}
      <div className="flex flex-col h-full w-full overflow-x-hidden">
        {/* Topo do Sidebar - Fixo, Sem Scroll e Perfeitamente Alinhado */}
        <div className="h-16 px-3.5 flex items-center border-b border-slate-800 relative w-full overflow-hidden shrink-0">
          <div className="flex items-center gap-3 w-full min-w-0">
            {/* Ícone do Logo - Fixo e Imutável */}
            ...
            {/* Texto do Logo - Ocupa o espaço restante sem empurrar nada */}
        ...
        <nav className="flex-1 px-3 py-4 space-y-1.5 overflow-y-auto overflow-x-hidden custom-scrollbar">
          {navItems.map((item, index) => {
            if (item.divider) {
              return (
                <div key={index} style={{ margin: "12px 8px" }}>
                  <div
                    style={{
                      height: "1px",
